//*********************************************************************************************************//
//****** This Application uses MQTT protocol to send data to the mosquitto broker in your local wifi ******//
//*********************************************************************************************************//
#include <Arduino.h>
#include <string>
#include <WiFi.h>
...
// MQTT Broker config
const char Broker_server[] = "192.168.1.107";
...
void loop() {
  ...
  // using a normal publish function and set the retain message flag to true so a new subscriber get the last message published about this topic
  pubState = mqtt_client.publish("esp32/hallSensor",(const uint8_t*) msg, strlen(msg), true);
