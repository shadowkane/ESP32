...
void setup() {
    ...
    // Set up mDNS (Multicast DNS)
    if(!MDNS.begin("myWiFi")){
      Serial.println("Error Seeting mDns");
      while(1){
        delay(1000);
      }
    }
    Serial.println("mDNS response starded");
...
void loop() {
  ...
  if(client){
    ...
    //while(client.connected()){
      ...
      // response start with header
      // header contains the status of the request. 200 OK means request has succeeded
      client.println("HTTP/1.1 200 OK");
      client.println("Content-type:text/html");
      client.println();
      client.println("<a href=\"/LogIn\">LogIn</a>");
      client.println();
    //}
